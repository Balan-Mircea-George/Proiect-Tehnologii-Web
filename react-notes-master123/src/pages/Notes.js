...
export default class Notes extends Component{
  ...
  <div className="px-4 mb-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
    {this.state.notes.filter(note => {
      ...
      <p className="text-gray-600 text-sm">{note.body}
      </p>
      <button className="button" onClick={this.handleLikeChange.bind(this)}>
      <span>🎉</span>
      <span>Like</span>
      <span value={this.state.like}>{this.state.like}</span>
    </button>
    </Link>
    ))}
    <button className="button" onClick={this.handleLikeChange.bind(this)}>
